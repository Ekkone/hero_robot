...
void CAN_Send_Referee_B( CAN_HandleTypeDef * hcan)
{
      ...
      TX_REFEREE_B[4] = Robot.heat.shoot_42_heat>>8;	//14mm枪口热
      ...
      TX_REFEREE_B[7] = (uint16_t)Robot.Chassis_Power.chassis_Power;
			...
			HAL_CAN_AddTxMessage(hcan, &CANSend_DP, TX_REFEREE_B, (uint32_t *)CAN_TX_MAILBOX0 );
}
...
void CAN_Send_MINI_B( CAN_HandleTypeDef * hcan)
{
	    CANSend_Referee_B.DLC = 0x08;
			CANSend_Referee_B.IDE = CAN_ID_STD;
			CANSend_Referee_B.RTR = CAN_RTR_DATA;
			CANSend_Referee_B.StdId = CAN_Mini_B;
      ...
      TX_DATA_MINI_B[7] = 0;
			...
			HAL_CAN_AddTxMessage(hcan, &CANSend_DP, TX_DATA_MINI_B, (uint32_t *)CAN_TX_MAILBOX0 );
}
//小云台裁判系统数据
uint8_t TX_DATA_S[8];
void CAN_Send_Referee_S( CAN_HandleTypeDef * hcan)
{
      ...
      TX_DATA_S[7] = 0;
			...
			HAL_CAN_AddTxMessage(hcan, &CANSend_DP, TX_DATA_S, (uint32_t *)CAN_TX_MAILBOX0 );
}
//小云台视觉数据以及控制数据
...
void CAN_Send_MINI_S( CAN_HandleTypeDef * hcan)
{
	    CANSend_Referee_B.DLC = 0x08;
			CANSend_Referee_B.IDE = CAN_ID_STD;
			CANSend_Referee_B.RTR = CAN_RTR_DATA;
			CANSend_Referee_B.StdId = CAN_Mini_S;
      ...
      TX_DATA_MINI_S[7] = 0;
			...
			HAL_CAN_AddTxMessage(hcan, &CANSend_DP, TX_DATA_MINI_S, (uint32_t *)CAN_TX_MAILBOX0 );
}